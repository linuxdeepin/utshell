...
extern int r_alias_builtin ( register WORD_LIST *list);
     ...
     WORD_LIST *list;
{
  ...
  pflag = 0;
  reset_internal_getopt (); /*lhead = lcurrent = loptend = (WORD_LIST *)NULL; sp = 1;*/
  ...
  if (list == 0 || pflag)
	...
	print_alias (alias_list[offset], dflags);
      ...
      if (list == 0)
	return (sh_chkwrite (EXECUTION_SUCCESS));//flush
    }
  ...
  while (list)
	...
	{
	  name[offset] = '\0';//need cp to transfrom rust
	  value = name + offset + 1;//need cp to transfrom rust
...
extern int r_unalias_builtin ( register WORD_LIST *list);
int
unalias_builtin (list)
     register WORD_LIST *list;
{
return r_unalias_builtin (list);
