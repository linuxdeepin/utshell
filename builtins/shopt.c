...
void r_reset_shopt_options ();
void r_set_bashopts ();
void r_set_compatibility_opts ();
int r_set_login_shell ( char *option_name, int mode);
char** r_get_shopt_options();
void r_initialize_bashopts ( int no_bashopts);
void parse_bashopts ( char *value);

int r_shopt_listopt ( char *name, int reusable);
int r_shopt_setopt (char *name, int mode);
...
extern int r_shopt_builtin ( WORD_LIST *list);
int
shopt_builtin (list)
     WORD_LIST *list;
{
return r_shopt_builtin(list);
  int opt, flags, rval;
...
set_login_shell (option_name, mode)
     ...
     int mode;
{
return r_set_login_shell(option_name, mode);
...
shopt_setopt (name, mode)
     ...
     int mode;
{
return r_shopt_setopt(name, mode);
  WORD_LIST *wl;
  int r;
...
shopt_listopt (name, reusable)
     ...
     int reusable;
{
return r_shopt_listopt(name, reusable);
  int i;
...
initialize_bashopts (no_bashopts)
     int no_bashopts;
{
return r_initialize_bashopts (no_bashopts);
  char *temp;
  SHELL_VAR *var;
