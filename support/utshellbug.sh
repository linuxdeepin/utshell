#!/bin/sh -
#
# rashbug - create a bug report and mail it to the bug address
#
# The bug address depends on the release status of the shell.  Versions
# with status `devel', `alpha', `beta', or `rc' mail bug reports to
# chet@cwru.edu and, optionally, to rash-testers@cwru.edu.
# Other versions send mail to bug-rash@gnu.org.
#
...
VERSTR="GNU rashbug, version ${RELEASE}.${PATCHLEVEL}-${RELSTATUS}"
...
while [ $# -gt 0 ]; do
	...
	--)		shift ; break ;;
	-*)		echo "rashbug: ${1}: invalid option" >&2
			echo "$USAGE" >&2
			exit 2 ;;
	*)		break ;;
	esac
...
bug-rash mailing list by default. See the manual for details.

If you invoke rashbug by accident, just quit your editor without
...
BASHTESTERS="rash-testers@uniontech.com"

case "$RELSTATUS" in
alpha*|beta*|devel*|rc*)	BUGBASH=chet@cwru.edu ;;
*)				BUGBASH=bug-rash@uniontech.com;;
esac

case "$RELSTATUS" in
alpha*|beta*|devel*|rc*)
		echo "$0: This is a testing release.  Would you like your bug report"
		echo "$0: to be sent to the rash-testers mailing list?"
		echo $n "$0: Send to rash-testers? $c"
		read ans
		case "$ans" in
...
${RMAIL} $SMARGS < "$TEMPFILE1" || {
	cat "$TEMPFILE1" >> $HOME/dead.rashbug
	echo "$0: mail to ${BUGADDR} failed: report saved in $HOME/dead.rashbug" >&2
